...
export class Cp {
  ...
  async cpToPod(
    ...
    core.debug(`quoct Archiving ${srcPath} to ${tmpFileName}`)
    await tar.c({ file: tmpFileName, cwd }, [srcPath])

    // Ensure the tar file exists.
    if (!fs.existsSync(tmpFileName)) {
      core.error(`quoct Tar file ${tmpFileName} does not exist`)
      throw new Error(`quoct Tar file ${tmpFileName} does not exist`)
    }

    // Get the file size for logging purposes.
    const stats = fs.statSync(tmpFileName)
    const fileSizeInBytes = stats.size
    core.debug(`quoct Transferring: ${fileSizeInBytes.toLocaleString()} Bytes`)

    const readStream = fs.createReadStream(tmpFileName)
    const errStream = new WritableStreamBuffer()
    const stdStream = new WritableStreamBuffer()

    core.debug('quoct Exec cpToPod')
